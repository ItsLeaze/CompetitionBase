...
#define SPAWN_FILE_SELECT_FILE_BUTTON(parent, saveFile)                                                \
    spawn_object_rel_with_rot((parent),                                                                \
                              (save_file_exists(saveFile) ? MODEL_MAIN_MENU_MARIO_SAVE_BUTTON          \
                                                          : MODEL_MAIN_MENU_MARIO_NEW_BUTTON),         \
                              bhvMenuButton, sSaveFileButtonPositions[saveFile][0],                    \
                              sSaveFileButtonPositions[saveFile][1],                                   \
                              sSaveFileButtonPositions[saveFile][2], 0x0, -0x8000, 0x0)
...
void hide_submenu_buttons(s16 prevMenuButtonID) {
    switch (prevMenuButtonID) {
        case MENU_BUTTON_ERASE:
            delete_menu_button_objects(MENU_BUTTON_ERASE_MIN, MENU_BUTTON_ERASE_MAX);
            break;
    }
}
...
#define SPAWN_FILE_SELECT_FILE_BUTTON_INIT(saveFile)                                                   \
    spawn_object_rel_with_rot(o,                                                                       \
                              (save_file_exists(saveFile) ? MODEL_MAIN_MENU_MARIO_SAVE_BUTTON_FADE     \
                                                          : MODEL_MAIN_MENU_MARIO_NEW_BUTTON_FADE),    \
                              bhvMenuButton, sSaveFileButtonInitPositions[saveFile][0],                \
                              sSaveFileButtonInitPositions[saveFile][1],                               \
                              sSaveFileButtonInitPositions[saveFile][2], 0x0, 0x0, 0x0)
...
void print_save_file_star_count(s8 fileIndex, s16 x, s16 y) {
    ...
    if (save_file_exists(fileIndex)) {
        s16 starCount = save_file_get_total_star_count(fileIndex, COURSE_NUM_TO_INDEX(COURSE_MIN),
...
void print_main_menu_strings(void) {
    ...
    print_hud_lut_string(HUD_LUT_DIFF, HEADER_X, 35, textSelectFile);
